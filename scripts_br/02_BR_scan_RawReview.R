...
scan <- googledrive::as_id("https://drive.google.com/drive/folders/1pbRqXX5NqSgS4Roh0Ap9gsP9SicKX5-A")
...
scan_list[[1]] <- scan_list[[1]][-c(1:10), ]

#second data frame:
scan_list[2]

scan_list[[2]] <- scan_list[[2]][-c(1:5), ]
...
for (i in seq_along(scan_list)) {
  ...
  # plot
  p <- ggplot(data = df) + 
    geom_line(aes(x=DateTime, y=Temperature...C....Measured.value, color='Temperature')) +
...
BRMQ4 <- scan_list$BRMQ4_absparams.csv
BRM06 <- scan_list$BRM06_absparams.csv
...
plot_variables <- function(df, file_name) {
  ...
  df_long <- df %>%
    dplyr::select("DateTime", "Temperature...C....Measured.value", "TSS_mg.l", "TOC_mg.l", "NO3_mg.l", "DOCeq_mg.l") %>%
