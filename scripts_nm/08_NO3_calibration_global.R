##==============================================================================
## Project: QuEST
## Nitrate Sensor Calibration: s::can global calibration -> grab sample correction
## Following Arial's s::can guide
##
## Global sensor column : NO3.N_mg.l
## Grab sample column   : NO3..mg.N.L.
##
## Models fitted per site:
##   (a) Linear          : NO3 grab ~ a + b * NO3 sensor
##   (b) Log-log (power) : log(NO3 grab) ~ log(NO3 sensor)  =>  NO3 = a * sensor^b
##   (c) Polynomial      : NO3 grab ~ sensor + sensor^2
...
# Extract NO3 as xts time series objects
...
grab_USF20 <- grab_USF20 %>%
  mutate(NO3..mg.N.L. = ifelse(Date == as.POSIXct(c("2024-05-23","2024-06-19")), NA, NO3..mg.N.L.))

# Remove problematic samples 
grab_USF12 <- grab_USF12 %>%
  mutate(NO3..mg.N.L. = ifelse(Date == as.POSIXct(c("2025-09-01")), NA, NO3..mg.N.L.))

# Quick scatter: raw sensor NO3 vs. grab NO3
...
ggplot(grab_USF12, aes(x = NO3..mg.N.L., y = NO3.N_mg.l)) +
  ...
  labs(title = "USF12: Raw sensor vs. grab NO3",
       x = "Grab NO3 (mg N/L)", y = "s::can NO3 (mg N/L)") +
  theme_minimal()

ggplot(grab_USF20, aes(x = NO3..mg.N.L., y = NO3.N_mg.l)) +
  geom_point(color = "blue") +
  geom_text(aes(label = format(DateTime, "%Y-%m-%d")), vjust = -0.5, size = 3) +
  labs(title = "USF20: Raw sensor vs. grab NO3",
       x = "Grab NO3 (mg N/L)", y = "s::can NO3 (mg N/L)") +
  theme_minimal()
...
ggplot(grab_USF21, aes(x = NO3..mg.N.L., y = NO3.N_mg.l)) +
  geom_point(color = "blue") +
  geom_text(aes(label = format(DateTime, "%Y-%m-%d")), vjust = -0.5, size = 3) +
  labs(title = "USF21: Raw sensor vs. grab NO3",
       x = "Grab NO3 (mg N/L)", y = "s::can NO3 (mg N/L)") +
  theme_minimal()
...
#### STEP 4: Run models ####
############################
# NOTE: in all models the RESPONSE is grab NO3 (the "truth") and the
# PREDICTOR is the sensor NO3 (what you have continuously).
...
grab_USF12_fit <- grab_USF12 %>% filter(!is.na(NO3..mg.N.L.), !is.na(NO3.N_mg.l))
grab_USF20_fit <- grab_USF20 %>% filter(!is.na(NO3..mg.N.L.), !is.na(NO3.N_mg.l))
grab_USF21_fit <- grab_USF21 %>% filter(!is.na(NO3..mg.N.L.), !is.na(NO3.N_mg.l))

##########################
#### (a) Linear model ####
##########################
# NO3_grab = a + b * NO3_sensor

## USF12 ##
linear_model12 <- lm(NO3..mg.N.L. ~ NO3.N_mg.l, data = grab_USF12_fit)
summary(linear_model12)

## USF20 ##
linear_model20 <- lm(NO3..mg.N.L. ~ NO3.N_mg.l, data = grab_USF20_fit)
summary(linear_model20)

## USF21 ##
linear_model21 <- lm(NO3..mg.N.L. ~ NO3.N_mg.l, data = grab_USF21_fit)
summary(linear_model21)

####################################
#### (b) Log-log (power) model  ####
####################################
# Fit: log(NO3_grab) = a + b * log(NO3_sensor)  =>  back-transformed: NO3 = exp(a) * sensor^b
# Good if the relationship curves upward or variance grows with concentration

## USF12 ##
log_model12 <- lm(log(NO3..mg.N.L.) ~ log(NO3.N_mg.l), data = grab_USF12_fit)
summary(log_model12)
cat("USF12 power law: NO3 =", exp(coef(log_model12)[1]), "* sensor ^", coef(log_model12)[2], "\n")

## USF20 ##
log_model20 <- lm(log(NO3..mg.N.L.) ~ log(NO3.N_mg.l), data = grab_USF20_fit)
summary(log_model20)
cat("USF20 power law: NO3 =", exp(coef(log_model20)[1]), "* sensor ^", coef(log_model20)[2], "\n")

## USF21 ##
log_model21 <- lm(log(NO3..mg.N.L.) ~ log(NO3.N_mg.l), data = grab_USF21_fit)
summary(log_model21)
cat("USF21 power law: NO3 =", exp(coef(log_model21)[1]), "* sensor ^", coef(log_model21)[2], "\n")

##############################
#### (c) Polynomial model ####
##############################
# NO3_grab = a + b*sensor + c*sensor^2
...
## USF12 ##
poly_model12 <- lm(NO3..mg.N.L. ~ NO3.N_mg.l + I(NO3.N_mg.l^2), data = grab_USF12_fit)
summary(poly_model12)

## USF20 ##
poly_model20 <- lm(NO3..mg.N.L. ~ NO3.N_mg.l + I(NO3.N_mg.l^2), data = grab_USF20_fit)
summary(poly_model20)

## USF21 ##
poly_model21 <- lm(NO3..mg.N.L. ~ NO3.N_mg.l + I(NO3.N_mg.l^2), data = grab_USF21_fit)
...
## USF12 ##
weighted_model12 <- lm(NO3..mg.N.L. ~ NO3.N_mg.l,
                       weights = 1 / NO3.N_mg.l^2,
                       data = grab_USF12_fit)
summary(weighted_model12)

## USF20 ##
weighted_model20 <- lm(NO3..mg.N.L. ~ NO3.N_mg.l,
                       weights = 1 / NO3.N_mg.l^2,
                       data = grab_USF20_fit)
summary(weighted_model20)

## USF21 ##
weighted_model21 <- lm(NO3..mg.N.L. ~ NO3.N_mg.l,
                       weights = 1 / NO3.N_mg.l^2,
                       data = grab_USF21_fit)
...
plot_calib_fits <- function(grabs, models, model_names, log_models = NULL, site) {
  x_seq <- seq(min(grabs$NO3.N_mg.l, na.rm = TRUE),
               max(grabs$NO3.N_mg.l, na.rm = TRUE), length.out = 200)
  nd    <- data.frame(NO3.N_mg.l = x_seq)

  ribbon_df <- do.call(rbind, lapply(seq_along(models), function(i) {
    is_log <- !is.null(log_models) && model_names[i] %in% log_models
    p      <- predict(models[[i]], newdata = nd, interval = "prediction", level = 0.95)
    if (is_log) p <- exp(p)
    data.frame(NO3.N_mg.l = x_seq, fit = p[,"fit"],
               lo = p[,"lwr"], hi = p[,"upr"], Model = model_names[i])
  }))

  ggplot() +
    geom_ribbon(data = ribbon_df,
                aes(x = NO3.N_mg.l, ymin = lo, ymax = hi, fill = Model), alpha = 0.15) +
    geom_line(data  = ribbon_df,
              aes(x = NO3.N_mg.l, y = fit, color = Model), linewidth = 0.9) +
    geom_point(data = grabs,
               aes(x = NO3.N_mg.l, y = NO3..mg.N.L.), color = "black", size = 2.5) +
    geom_text(data  = grabs,
              aes(x = NO3.N_mg.l, y = NO3..mg.N.L.,
    ...
    labs(title    = paste0(site, ": Calibration model fits"),
         ...
         x = "s::can global NO3 (mg N/L)", y = "Grab NO3 (mg N/L)") +
...
###################################
# Adds predicted NO3 columns + 95% prediction interval (PI) bounds
# to the full sensor time series for every model.

## USF12 ##
nd12 <- data.frame(NO3.N_mg.l = USF12$NO3.N_mg.l)
...
## USF20 ##
nd20 <- data.frame(NO3.N_mg.l = USF20$NO3.N_mg.l)
...
## USF21 ##
nd21 <- data.frame(NO3.N_mg.l = USF21$NO3.N_mg.l)
...
plot_pred_vs_obs <- function(grabs, site_df, site) {
  ...
  ggplot(df, aes(x = NO3..mg.N.L., y = Predicted, color = Model)) +
    ...
    labs(title    = paste0(site, ": Predicted vs. Observed (grab samples)"),
         ...
         x = "Observed grab NO3 (mg N/L)", y = "Predicted NO3 (mg N/L)") +
    theme_minimal(base_size = 12) +
...
plot_residuals <- function(grabs, site_df, site) {
  df <- grabs %>%
    ...
    pivot_longer(cols = c(pred_linear, pred_logpower, pred_poly, pred_weighted),
                 names_to = "Model", values_to = "Predicted") %>%
    mutate(Residual = NO3..mg.N.L. - Predicted)
  ...
  ggplot(df, aes(x = Predicted, y = Residual, color = Model)) +
    ...
    labs(title    = paste0(site, ": Residuals"),
         ...
         x = "Predicted NO3 (mg N/L)", y = "Residual (Observed - Predicted)") +
    theme_minimal(base_size = 12) +
    theme(legend.position = "none")
}
...
## USF12 ##
df_long12 <- USF12 %>%
  select(DateTime, NO3.N_mg.l, pred_linear, pred_logpower, pred_poly, pred_weighted) %>%
  pivot_longer(cols = -DateTime, names_to = "Variable", values_to = "Value") %>%
  mutate(Variable = recode(Variable,
                           "NO3.N_mg.l"    = "Raw sensor",
                           "pred_linear"   = "Linear",
                           "pred_logpower" = "Log-Power",
                           "pred_poly"     = "Polynomial",
                           "pred_weighted" = "Weighted"))

df_grabs12 <- grab_USF12 %>% filter(!is.na(NO3..mg.N.L.))
...
ggplot() +
  ...
  scale_linetype_manual(values = c("Raw sensor" = "dashed", "Linear" = "solid",
                                   ...
                                   "Weighted"   = "solid")) +
  labs(title    = "USF12: NO3 time series — all models",
       subtitle = "Shaded band = 95% PI (linear)  |  Open circles = grab samples  |  Dashed = raw sensor",
       x = "Date", y = "NO3 (mg N/L)", color = NULL, linetype = NULL) +
...
df_grabs20 <- grab_USF20 %>% filter(!is.na(NO3..mg.N.L.))
...
ggplot() +
  ...
  scale_linetype_manual(values = c("Raw sensor" = "dashed", "Linear" = "solid",
                                   "Log-Power"  = "solid",  "Polynomial" = "solid",
                                   "Weighted"   = "solid")) +
  labs(title    = "USF20: NO3 time series — all models",
       subtitle = "Shaded band = 95% PI (linear)  |  Open circles = grab samples  |  Dashed = raw sensor",
       x = "Date", y = "NO3 (mg N/L)", color = NULL, linetype = NULL) +
  scale_x_datetime(date_breaks = "2 weeks", date_labels = "%b %d") +
  theme_minimal(base_size = 12) +
...
df_grabs21 <- grab_USF21 %>% filter(!is.na(NO3..mg.N.L.))
...
ggplot() +
  ...
  scale_linetype_manual(values = c("Raw sensor" = "dashed", "Linear" = "solid",
                                   "Log-Power"  = "solid",  "Polynomial" = "solid",
                                   "Weighted"   = "solid")) +
  labs(title    = "USF21: NO3 time series — all models",
       subtitle = "Shaded band = 95% PI (linear)  |  Open circles = grab samples  |  Dashed = raw sensor",
       x = "Date", y = "NO3 (mg N/L)", color = NULL, linetype = NULL) +
  scale_x_datetime(date_breaks = "2 weeks", date_labels = "%b %d") +
  theme_minimal(base_size = 12) +
...
######################################
# Keep only: DateTime, global sensor NO3, grab sample NO3, grab identifier
# columns (Date, Site), and all model predictions + prediction intervals

USF12 <- USF12 %>%
  select(DateTime, NO3.N_mg.l, Date, Site,
         NO3..mg.N.L.,
...
#### Save in Drive ####
#######################
write.csv(USF12, file = "predicted/PredictedNO3_USF12_global.csv")
write.csv(USF20, file = "predicted/PredictedNO3_USF20_global.csv")
write.csv(USF21, file = "predicted/PredictedNO3_USF21_global.csv")
...
drive_upload(media = "predicted/PredictedNO3_USF12_global.csv", path = as_id(drive_folder_id))
drive_upload(media = "predicted/PredictedNO3_USF20_global.csv", path = as_id(drive_folder_id))
drive_upload(media = "predicted/PredictedNO3_USF21_global.csv", path = as_id(drive_folder_id))
